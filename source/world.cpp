...
#if USE_SPINLOCK && !USE_MUTEX && !USE_THREADS && !USE_THREAD_POOL // USE_SPINLOCK
    void World::update_spnlck(float dt){
        update_hero_spnlck(dt);
        update_npcs_spnlck(dt);
        update_food_consumption_spnlck(dt);
        update_predators_spnlck(dt);
        update_reproduction_spnlck(dt);
        update_starvation_system_spnlck(dt);
        update_tiredness_system_spnlck(dt);
        update_food_generator_spnlck(dt);
        process_deferred_removals_spnlck();
    }

#endif // USE_SPINLOCK

#if USE_THREAD_POOL && !USE_MUTEX && !USE_THREADS && !USE_SPINLOCK // THREAD POOL WORLD UPDATE
    #include "thread_pool.h"
    static ThreadPool g_threadPool(4);
    static bool g_threadPoolStarted = false;

    void World::world_update_thread_pool(float dt) {
        // Start thread pool only once
        if (!g_threadPoolStarted) {
            g_threadPool.Start();
            g_threadPoolStarted = true;
        }

        // Submit all systems as tasks
        g_threadPool.Submit([this, dt] { this->update_hero_ts(dt); });
        g_threadPool.Submit([this, dt] { this->update_npcs_ts(dt); });
        g_threadPool.Submit([this, dt] { this->update_food_consumption_ts(dt); });
        g_threadPool.Submit([this, dt] { this->update_predators_ts(dt); });
        g_threadPool.Submit([this, dt] { this->update_reproduction_ts(dt); });
        g_threadPool.Submit([this, dt] { this->update_starvation_system_ts(dt); });
        g_threadPool.Submit([this, dt] { this->update_tiredness_system_ts(dt); });
        g_threadPool.Submit([this, dt] { this->update_food_generator_ts(dt); });
...
#if USE_MUTEX && !USE_THREADS && !USE_THREAD_POOL // USE_MUTEX
...
#if USE_THREAD_POOL && !USE_MUTEX && !USE_THREADS // THREAD_POOL
    // Implemented above
#elif !USE_THREADS && !USE_THREAD_POOL && !USE_MUTEX // BASE
...
void World::process_deferred_removals_spnlck() {
...
void World::update_hero_spnlck(float dt) {
    recursive_scoped_lock<spinlock_mutex> lock{
        g_worldSpinlock,
        spinlock_mtx_locked,
        spinlock_idle_opts::noop<10>{}
...
void World::update_npcs_spnlck(float dt) {
    recursive_scoped_lock<spinlock_mutex> lock{
        g_worldSpinlock,
        spinlock_mtx_locked,
        spinlock_idle_opts::noop<10>{}
...
void World::update_food_consumption_spnlck(float dt) {
    recursive_scoped_lock<spinlock_mutex> lock{
        g_worldSpinlock,
        spinlock_mtx_locked,
        spinlock_idle_opts::noop<10>{}
...
void World::update_predators_spnlck(float dt) {
    recursive_scoped_lock<spinlock_mutex> lock{
        g_worldSpinlock,
        spinlock_mtx_locked,
        spinlock_idle_opts::noop<10>{}
...
void World::update_starvation_system_spnlck(float dt) {
    recursive_scoped_lock<spinlock_mutex> lock{
        g_worldSpinlock,
        spinlock_mtx_locked,
        spinlock_idle_opts::noop<10>{}
...
void World::update_tiredness_system_spnlck(float dt) {
    recursive_scoped_lock<spinlock_mutex> lock{
        g_worldSpinlock,
        spinlock_mtx_locked,
        spinlock_idle_opts::noop<10>{}
...
void World::update_reproduction_spnlck(float dt) {
    recursive_scoped_lock<spinlock_mutex> lock{
        g_worldSpinlock,
        spinlock_mtx_locked,
        spinlock_idle_opts::noop<10>{}
...
void World::update_food_generator_spnlck(float dt) {
    recursive_scoped_lock<spinlock_mutex> lock{
        g_worldSpinlock,
        spinlock_mtx_locked,
        spinlock_idle_opts::noop<10>{}
